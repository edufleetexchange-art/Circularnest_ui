...
// For Vercel: use absolute URL if on production domain, otherwise use relative path for dev
const getAPIBaseURL = (): string => {
  const env = import.meta.env.VITE_API_URL;
  
  // If explicitly set, use it (Vercel production or explicit configuration)
  if (env) {
    return env;
  }
  
  // Use relative /api path - Vite proxy handles forwarding in dev, same-origin in production
  // This ensures the Vite dev server proxy works correctly
  return '/api';
};
...
class ApiService {
  ...
  async updateCircularStatus(id: string, status: 'pending' | 'approved' | 'rejected') {
    const response = await this.api.put(`/circulars/${id}/status`, { status });
    return response.data;
  }
  ...
  async updateCircular(id: string, data: {
    ...
    const response = await this.api.put(`/circulars/${id}`, data);
    return response.data;
  }
  ...
  async submitGuestUpload(formData: FormData) {
    const response = await this.api.post('/pending/guest-upload', formData, {
      headers: {
        'Content-Type': 'multipart/form-data'
      }
    });
  ...
  async getPendingUploads(status?: string) {
    const response = await this.api.get('/pending', { params: { status } });
    return response.data;
  }

  async getMySubmissions() {
    const response = await this.api.get('/pending/my-submissions');
    return response.data;
  }

  async approvePendingUpload(id: string, reviewNotes?: string) {
    const response = await this.api.put(`/pending/${id}/approve`, { reviewNotes });
    return response.data;
  }

  async rejectPendingUpload(id: string, reviewNotes: string) {
    const response = await this.api.put(`/pending/${id}/reject`, { reviewNotes });
    return response.data;
  }

  async deletePendingUpload(id: string) {
    const response = await this.api.delete(`/pending/${id}`);
    return response.data;
  }
